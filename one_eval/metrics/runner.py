    ...
    def _run_metric_parallel(self, fn, preds: List[Any], refs: List[Any], kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for res in results:
            ...
            if "details" in res and isinstance(res["details"], list):
                merged_details.extend(res["details"])
        ...
        if merged_details:
            ...
            return {
                ...
                # Artifacts from chunks are hard to merge generically, so we drop them or take first?
                # For now, drop artifacts in parallel mode or merge if possible.
                # Most parallelizable metrics (rouge, acc) don't have critical global artifacts.
            }
        else:
        ...
        for cfg in metrics_cfg:
            ...
            try:
                if name not in NON_PARALLEL_METRICS and len(preds) > 100:
                    ...
                    res = self._run_metric_parallel(fn, preds, refs, (cfg.get("args", {}) or {}))
                else:
